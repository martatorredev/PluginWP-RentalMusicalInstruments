 ...
 * Plantilla Vista de pagar la liquidación del alquiler de un producto (pagina del checkout o pagina de pago).
 * File: html-wcai-liquidar-checkout-page.php
 */
...
<form name="checkout" id="checkout" method="post" class="checkout woocommerce-checkout" action="" 
	...
	<!--- AQUI COMIENZA EL CUADRO DE LA LIQUIDACION DEL ALQUILER EXCLUSIVO PARA ESTE PLUGIN  -->
...
<!--- AQUI TERMINA EL CUADRO DE LA LIQUIDACION DEL ALQUILER EXCLUSIVO PARA ESTE PLUGIN  -->		
	 <?php  woocommerce_checkout_payment(); 	
	 
	 // Obtiene los subtotales y totales del carrito de alquiler
	 $totales_liquidacion = WC()->cart->get_totals(); 
	 $subtotal_liquidacion = $totales_liquidacion['subtotal'];
	 $subtotal_tax = $totales_liquidacion['subtotal_tax'];
	 $total_liquidacion = $subtotal_liquidacion + $subtotal_tax;  
	 $shipping_total = $totales_liquidacion['shipping_total'];
...
<!--- Otros datos adicionales -->      
      ...
      <input type="hidden" name="subtotal" value='<?php echo $subtotal_liquidacion ; ?>' />
      <input type="hidden" name="total_tax" value='<?php echo $subtotal_tax ; ?>' />
      <input type="hidden" name="total" value='<?php echo $total_liquidacion ; ?>' />
      <input type="hidden" name="shipping_total" value='<?php echo  $shipping_total; ?>' />
      <input type="hidden" name="nombre_liquidacion" value='<?php echo  $nombre_liquidacion; ?>' />
      <input type="hidden" name="nro_orden_liquidar" value='<?php echo  $nro_orden; ?>' />
      <input type="hidden" name="wcai_cart_subtotal" value='<?php echo   WC()->cart->get_displayed_subtotal() ; ?>' />
        
      
      <?php //print_r( $totales_liquidacion ); ?>'
              ...
              //Oculta el payment box
               ...
               if (document.checkout.payment_method.checked) {
                    ...
                    if (pay_box_selected == "realex_redirect") {
                        document.getElementById("checkout").action = "<?php echo SITE_URL . '/liquidar-alquiler';?>";
                    }
               }
               return;
          }
